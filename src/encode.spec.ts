...
describe("encodeSync", () => {
	...
	test("Temporal Duration", { skip: !supportsTemporal() }, () => {
	...
	test("Temporal PlainDate", { skip: !supportsTemporal() }, () => {
	...
	test("Temporal PlainDateTime", { skip: !supportsTemporal() }, () => {
	...
	test("Temporal PlainMonthDay", { skip: !supportsTemporal() }, () => {
	...
	test("Temporal ZonedDateTime", { skip: !supportsTemporal() }, () => {
...
describe("encode", () => {
	...
	test("array of Temporal values", { skip: !supportsTemporal() }, async () => {
		const values = [
		...
		expect(await quickEncode(values)).toBe(
			`[a\"PT1H30M\",p\"2021-03-11T16:00:00Z\",d\"2021-07-01\",q\"2021-07-01T12:34:56\",m\"05-02\",T\"12:34:56\",y\"2021-05\",Z\"2021-07-01T12:34:56-04:00[America/New_York]\"]\n`,
		);
	});
...
describe("highWaterMark", () => {
	async function collectChunks(
		...
		const stream = encode(value, { highWaterMark });
		const chunks: string[] = [];
		const reader = stream.getReader();
		try {
			while (true) {
	...
	test("default highWaterMark batches small values into fewer chunks", async () => {
		// Create an object with many small keys — total encoded output is small
		// but without buffering would produce many controller.enqueue calls
		const obj: Record<string, number> = {};
		for (let i = 0; i < 100; i++) {
			obj[`k${i}`] = i;
		}
		const chunks = await collectChunks(obj);
		// With the default 16KB highWaterMark the entire payload fits in one chunk
		expect(chunks.length).toBe(1);
	});

	test("highWaterMark of 1 produces more chunks for promises", async () => {
		// With a very low highWaterMark each flush produces its own chunk
		const value = [
			Promise.resolve("a"),
			Promise.resolve("b"),
			Promise.resolve("c"),
		];
		const chunksLow = await collectChunks(value, 1);
		const chunksDefault = await collectChunks(value);
		// A low highWaterMark should produce at least as many chunks as the default
		expect(chunksLow.length).toBeGreaterThanOrEqual(chunksDefault.length);
		// And the concatenated results should be identical
		expect(chunksLow.join("")).toBe(chunksDefault.join(""));
	});

	test("highWaterMark of 1 produces more chunks for async iterables", async () => {
		const asyncIterable = {
			async *[Symbol.asyncIterator]() {
				yield 1;
				yield 2;
		...
		const chunksLow = await collectChunks(asyncIterable, 1);
		const chunksDefault = await collectChunks(asyncIterable);
		expect(chunksLow.length).toBeGreaterThanOrEqual(chunksDefault.length);
		expect(chunksLow.join("")).toBe(chunksDefault.join(""));
	});

	test("very large highWaterMark produces fewer chunks than small", async () => {
		const value = [
			Promise.resolve(1),
			Promise.resolve(2),
			Promise.resolve(3),
		];
		const chunksLarge = await collectChunks(value, 1024 * 1024);
		const chunksSmall = await collectChunks(value, 1);
		// A large highWaterMark should produce fewer (or equal) chunks than a tiny one
		expect(chunksSmall.length).toBeGreaterThanOrEqual(chunksLarge.length);
		expect(chunksLarge.join("")).toBe(chunksSmall.join(""));
	});

	test("highWaterMark does not affect correctness of encoded output", async () => {
		const value = {
			str: "hello",
			num: 42,
			nested: { a: [1, 2, 3] },
		...
		const hwms = [1, 16, 256, 16 * 1024, 1024 * 1024];
		const results = await Promise.all(
			hwms.map(async (hwm) => (await collectChunks(value, hwm)).join("")),
		);
		// All highWaterMark values produce identical concatenated output
...
describe("large payloads", () => {
	test("large TypedArrays without stack overflow", async () => {
		// 2gb
		const largeBuffer = new Uint8Array(1 * 1024 * 1024 * 1024);
		largeBuffer.fill(42);

		const stream = encode(largeBuffer);
		const reader = stream.getReader();
		let totalLength = 0;
		while (true) {
			const { done, value } = await reader.read();
		...
		expect(totalLength).toBeGreaterThan(128 * 1024);
	});
});
